paramNames <- c("start_capital", "annual_mean_return", "annual_ret_std_dev",
                "annual_inflation", "monthly_withdrawals"
                , "n_obs","n_sim")

simulate_nav <- function(start_capital = 1000000, annual_mean_return = 5.0,
        ...
        monthly.invest.returns[] = rnorm(n.obs * n.sim, mean = monthly.mean.return,
                                         sd = monthly.ret.std.dev)
        ...
        for (j in 1:n.obs) {
                nav[j + 1, ] = nav[j, ] * (1 + monthly.invest.returns
                   [j, ] - monthly.inflation.returns[j, ]) - monthly.withdrawals
        }
...
plot_nav <- function(nav) {
        ...
        palette(c("red","green","blue", "orange","violet"))
        ...
        plot(100* p.alive, las = 1, xlab = 'Months', ylab = 'Percentage Paying',
             main = 'Percentage of Paying Scenario',ylim =c(0,100))
        grid()
        ...
        plot(density(final.nav, from=0, to=max(final.nav)), las = 1, 
             xlab = 'Final Capital',
             main = paste0('Distribution of Final Capital\n', 100 *p.alive
                          [last.period], '% are still paying'))
        grid()
}
